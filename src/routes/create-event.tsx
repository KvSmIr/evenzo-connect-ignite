...
import { X, ArrowLeft, ImagePlus, MapPin, Check, AlertTriangle } from "lucide-react";
          ...
          <div className="space-y-4 animate-fade-in">
            ...
            <Field label="Titre de l'événement" required>
              <input
                ...
                placeholder="Ex: Soirée Afrobeats…"
                className="input"
              />
            </Field>
          ...
          <div className="space-y-4 animate-fade-in">
            <Field label={`Description (${description.length}/300)`}>
              <textarea
                ...
                rows={5}
                placeholder="Décris l'ambiance, les artistes…"
              ...
              <div className="grid grid-cols-2 gap-1 rounded-xl bg-secondary p-1">
                <button
                  ...
                  className={cn(
                    ...
                    isFree ? "bg-card text-foreground shadow-card" : "text-muted-foreground"
                  )}
                >
                  Gratuit
                </button>
          ...
          <div className="space-y-5 animate-fade-in">
            ...
            <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
              <div className="relative h-[180px] bg-secondary">
                ...
                <div className="absolute bottom-3 left-3 right-3">
                  <p
                    className="text-[12px] font-semibold tracking-wider text-primary"
                    style={{ textShadow: "0 1px 4px rgba(0,0,0,0.8)" }}
                  >
                    {date && new Date(date).toLocaleDateString("fr-FR", { weekday: "long" }).toUpperCase()}{" "}
                    · {time}
                  </p>
                  <h3
            ...
            <div className="space-y-2 rounded-xl border border-border bg-card p-4">
              {[
                ...
                <div key={c.l} className="flex items-center gap-2 text-sm">
                  <Check
                    className={cn("h-4 w-4", c.ok ? "text-success" : "text-muted-foreground")}
                  />
            ...
            <div className="flex gap-3 rounded-xl border border-primary/30 bg-primary/10 p-3">
              ...
              <p className="text-xs leading-relaxed text-foreground">
                En publiant, tu confirmes que cet événement est réel. Tout faux événement entraîne
            ...
            <button
              ...
              disabled={busy}
              className="h-13 w-full rounded-lg bg-gradient-flame py-3.5 text-base font-bold text-primary-foreground shadow-flame transition-transform active:scale-[0.98] disabled:opacity-60"
            >
      ...
      {step < 3 && (
        <div className="fixed bottom-0 left-1/2 z-30 w-full max-w-[430px] -translate-x-1/2 border-t border-border bg-background/95 p-4 pb-safe backdrop-blur-xl">
          <button
            ...
            className="h-12 w-full rounded-lg bg-gradient-flame text-base font-bold text-primary-foreground shadow-flame transition-opacity disabled:opacity-40"
          >
            Continuer
          </button>
        </div>
